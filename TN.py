...
class Vehicle:
    def __init__(self, id, origin, destination, distance, road, next_road, path, Emax, Ewait, Edrive, iswait, charge):
        self.id = id
        self.origin = origin
    ...
    def drive(self, road):
        drive_distance = road.calculate_drive(road.destination)
    ...
    def wait(self, road, fr, to):
        ...
        if self.iswait == 0 and self.distance == 0:
            ...
            if self.iswait <= t:
                self.iswait = 0
                self.change_road()
                junction.wait.erase((self.id, self.iswait))
            else:
                self.iswait -= t
...
class Edge:
    def __init__(self, origin, destination, length, capacity, free_time, b, power):
    ...
    def calculate_drive(self, to):
        cap, x = self.capacity[to]
...
class Node:
    def __init__(self, id, signal, wait, ischarge, edge_num):
        #signal = {{fr, to}: {g, C}}, 表示每个方向信号灯所属绿灯时长与总周期时长
        #wait = {{vid, t}} 表示正在等候的车辆的合集
        self.id = id
        self.signal = signal
        self.wait = wait
        self.is_charge = ischarge
